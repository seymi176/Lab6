...
As shown below, it takes almost 0.36 seconds for the function *"brute_force_knapsack"* to find the solution for n = 16 objects.
...
By implementing the parallel method for the brute force function, the elapsed time reduced from 0.36 second for non-paralleled method to only 0.02 second.   

We also recognized that we have a time consuming process in for-loop, so by substituting lapply instead of for-loop we could reduce the elapsed time for the brute force function.   
